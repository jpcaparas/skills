#!/usr/bin/env python3
"""
validate.py — Validate the seo-analysis skill structure and conventions.
...
REQUIRED_SCRIPTS = ["build_fix_prompt.py", "probe_seo_analysis.py", "validate.py", "test_skill.py"]
REQUIRED_REFERENCES = [
...
def has_toc_heading(content: str) -> bool:
    patterns = [r"^#+\s+table\s+of\s+contents", r"^#+\s+toc\b", r"^#+\s+contents\b", r"^\-\s+\[.*\]\(#"]
...
def validate_skill(skill_path: str) -> dict:
    ...
    if not os.path.isfile(evals_path):
        ...
        evals_data = json.loads(open(evals_path, "r", encoding="utf-8").read())
        tags = {tag for item in evals_data.get("evals", []) for tag in item.get("tags", [])}
        for tag in ["smoke", "edge", "negative", "disclosure"]:
            if tag not in tags:
                errors.append(f"Missing eval coverage for tag: {tag}")

    template_path = os.path.join(skill_path, "templates", "fix-prompt-template.md")
    if not os.path.isfile(template_path):
        errors.append("Missing templates/fix-prompt-template.md")
