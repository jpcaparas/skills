...
Use a quick, token-efficient critic configuration by default and reserve expansive reasoning, context, turns, tool breadth, and token investment for build-related descendants. Give an ordinary critic enough capability to inspect the real artifact directly, but keep its assignment to a concise verdict, concrete evidence, and one highest-leverage gap. Do not fan out routine criticism. Escalate critic capability or add specialist critic descendants only when a concrete review need warrants it, and record that reason. This is adaptive allocation rather than a fixed numeric cap; if the quick configuration cannot inspect fairly, escalate or return `BLOCKED`. Never trade away artifact-grounded review merely to save tokens. Keep this envelope out of the prepared actual prompt and `artifact/PROMPT.md`.
