...
On the lead’s work, the skill imposes no time, token, step, tool-call, dependency, source-file-count, framework, language, asset-source, browser-tool, testing-strategy, iteration, subagent-count, or subagent-depth budget. It does not require goal mode or any equivalent harness feature. The fixed boundaries are the portable handoff and local-only publication authority described here.
