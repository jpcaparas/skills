...
Use a quick, token-efficient critic configuration by default. Reserve expansive reasoning, context, turns, tool breadth, and token investment for the lead and build-related descendants that research, create, debug, integrate, and verify the artifact. Give an ordinary critic only the sealed prompt, accepted bar, exact artifact revision, relevant references and constraints, prepared captures or tests, and enough tools and context to inspect the real artifact directly. Keep its work narrow: inspect, return a concise verdict with concrete evidence and one highest-leverage gap, then stop. Do not spend critic turns on implementation, open-ended redesign, broad exploratory research, repeated restatement, or generating the fix; those belong to build-related descendants. Do not fan out routine criticism; add specialist critic descendants only when a recorded review need warrants them.
