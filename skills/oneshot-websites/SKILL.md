...
This step is complete when each lead reports a finished drop-ready artifact or a genuine blocker, all outcome-relevant descendant branches are finished or honestly accounted for, the integrated result has been checked, all descendants and writes remain within the assigned experiment, and every `OK` run has deleted its `.tmp/` directory in its entirety while non-successful runs retain it for recovery.
...
After all leads finish, confirm every successful run lacks `.tmp/` and every non-successful recoverable run still has its exact ordinary `.tmp/`, then build and validate the catalogue:
