#!/usr/bin/env python3
"""Validate the maintainable-app skill package."""
...
def validate(root: Path) -> dict[str, object]:
    ...
    if skill_md.is_file():
        ...
        if "{{ skill:maintainable-tests }}" not in content:
            errors.append("SKILL.md must use symbolic reference to {{ skill:maintainable-tests }}")
    ...
    observability = root / "references" / "observability.md"
    if observability.is_file():
        obs_text = read_text(observability).lower()
        golden_index = obs_text.find("golden rule")
        metrics_index = obs_text.find("## metrics")
        if golden_index == -1 or metrics_index == -1 or golden_index < metrics_index:
            errors.append("references/observability.md must make the four golden signals a metrics golden rule")
