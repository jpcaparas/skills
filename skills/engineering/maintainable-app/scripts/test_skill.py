#!/usr/bin/env python3
"""Lightweight tests for the maintainable-app skill."""
...
def write_fixture(root: Path) -> None:
    src = root / "src"
    src.mkdir()
    (src / "jobs.ts").write_text(
        """
export async function startExport(userId: string) {
  const job = await db.jobs.create({ status: "pending", userId });
  await queue.dispatch("export", { userId });
  return job;
}
...
export async function processExport(job: any) {
  try {
    ...
    if (!response.ok) retry(job.id);
    console.log("export failed");
    metrics.counter("banana_count").add(1, { userId: job.userId });
  } catch (error) {
  }
}
""".lstrip(),
...
""".lstrip(),
    ...
    (src / "telemetry.ts").write_text(
        """
export function recordDecorativeCounter() {
  metrics.counter("banana_count").add(1, { userId: "123" });
}
""".lstrip(),
        encoding="utf-8",
    )
...
def main(argv: list[str]) -> int:
    ...
    help_check = subprocess.run(
        [sys.executable, str(root / "scripts" / "analyze_app_resilience.py"), "--help"],
        text=True,
        capture_output=True,
        check=False,
    )
    if help_check.returncode != 0 or "application resilience review prompts" not in help_check.stdout:
        errors.append("analyze_app_resilience.py --help did not return expected help text")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        scan = subprocess.run(
            [
                sys.executable,
                str(root / "scripts" / "analyze_app_resilience.py"),
                str(fixture_root),
                "--json",
            ],
            text=True,
        ...
        if scan.returncode != 0:
            errors.append("analyze_app_resilience.py failed on fixture")
        else:
    ...
    template = root / "templates" / "resilience-review.md"
    if not template.is_file():
        errors.append("resilience review template is missing")
