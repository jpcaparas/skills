#!/usr/bin/env python3
"""
validate.py - Validate the ripgrep skill structure and conventions.
...
def validate_skill(skill_path: str) -> dict[str, object]:
    ...
    required_files = [
        ...
        "scripts/probe_ripgrep.py",
        "scripts/validate.py",
        "scripts/test_skill.py",
        "templates/ripgreprc.example",
        "templates/rg-aliases.sh",
        "evals/evals.json",
        "assets/.gitkeep",
    ]
    ...
    for rel_path in ["scripts/probe_ripgrep.py", "scripts/validate.py", "scripts/test_skill.py"]:
    ...
    alias_template = root / "templates" / "rg-aliases.sh"
    if alias_template.is_file():
        shell_error = syntax_check_shell(alias_template)
        if shell_error:
            errors.append(f"Shell syntax error in templates/rg-aliases.sh: {shell_error}")
