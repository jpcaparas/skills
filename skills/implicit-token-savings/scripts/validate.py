#!/usr/bin/env python3
"""
validate.py - Validate the implicit-token-savings skill structure and conventions.
...
def validate_skill(skill_path: str) -> dict[str, object]:
    ...
    required_files = [
        ...
        "scripts/probe_implicit_token_savings.py",
        "scripts/validate.py",
    ...
    template_path = root / "templates" / "session-checklist.md"
    if template_path.is_file():
        shell_candidate = root / "templates" / "session-checklist.sh"
        if shell_candidate.is_file():
            shell_error = syntax_check_shell(shell_candidate)
            if shell_error:
                errors.append(f"Shell syntax error in templates/session-checklist.sh: {shell_error}")
