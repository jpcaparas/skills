...
Official source: https://www.chezmoi.io/user-guide/frequently-asked-questions/troubleshooting/

Branch: recovery

Site last modified: 2026-07-07

Article SHA-256: `439380edf22e606ed915571d1038bbacb0498d0b11e60a7702546191424bfc53`
...
- [A specific command is not behaving as I expect. How can I debug it?](#a-specific-command-is-not-behaving-as-i-expect-how-can-i-debug-it)
...
- [Why do I get a blank buffer or empty file when running `chezmoi edit`?](#why-do-i-get-a-blank-buffer-or-empty-file-when-running-chezmoi-edit)
...
- [chezmoi reports `chezmoi: fork/exec /tmp/XXXXXXXXXX.XX: exec format error` when executing a template script](#chezmoi-reports-chezmoi-forkexec-tmpxxxxxxxxxxxx-exec-format-error-when-executing-a-template-script)
- [chezmoi reports `chezmoi: fork/exec /tmp/XXXXXXXXXX.XX: permission denied` when executing a script](#chezmoi-reports-chezmoi-forkexec-tmpxxxxxxxxxxxx-permission-denied-when-executing-a-script)
- [chezmoi reports `chezmoi: mkdir xxxxx: no such file or directory` when trying to manage file or directory](#chezmoi-reports-chezmoi-mkdir-xxxxx-no-such-file-or-directory-when-trying-to-manage-file-or-directory)
- [chezmoi reports `read /dev/stdin: permission denied` or `write /dev/stdout: permission denied` when I redirect standard input or standard output](#chezmoi-reports-read-devstdin-permission-denied-or-write-devstdout-permission-denied-when-i-redirect-standard-input-or-standard-output)
- [chezmoi reports `fork/exec ...: no such file or directory` when running scripts on Nix or Termux](#chezmoi-reports-forkexec-no-such-file-or-directory-when-running-scripts-on-nix-or-termux)
...
TOML YAML JSON ~/.config/chezmoi/chezmoi.toml
...
Pass the `-f` flag, e.g. by setting the `edit.args` configuration variable to `["-f"]`, or by setting the `EDITOR` environment variable to include the `-f` flag, e.g. `export EDITOR="vim -f"`.

Pass the `--wait` flag, e.g. by setting the `edit.args` configuration variable to `["--wait"]` or by setting the `EDITOR` environment variable to include the `--wait` flag, e.g. `export EDITOR="code --wait"`.
...
This will apply to all files and directories that chezmoi manages and will ensure that none of them are group writeable. It is not currently possible to control group write permissions for individual files or directories. Please [open an issue on GitHub](https://github.com/twpayne/chezmoi/issues/new?assignees=&labels=enhancement&template=02_feature_request.md&title=) if you need this.

## chezmoi reports `chezmoi: user: lookup userid NNNNN: input/output error`

This is likely because the chezmoi binary you are using was statically compiled with [musl](https://musl.libc.org/) and the machine you are running on uses LDAP or NIS.
...
If the problem still persists, then please [open an issue on GitHub](https://github.com/twpayne/chezmoi/issues/new/choose) .
...
Under the hood, chezmoi uses [bbolt](https://github.com/etcd-io/bbolt) which permits multiple simultaneous readers, but only one writer (with no readers).
...
TOML YAML JSON ~/.local/share/chezmoi/.chezmoiexternal.toml
...
~/.local/share/chezmoi/.chezmoiexternal.yaml
...
~/.local/share/chezmoi/.chezmoiexternal.json
...
Now once that done `chezmoi add ~/.config/direnv/direnvrc` should work. For reference see [issue #2006](https://github.com/twpayne/chezmoi/issues/2006) .
...
This error occurs when you [installed chezmoi with snap](https://snapcraft.io/chezmoi) and is caused by a long-standing [bug in snap](https://bugs.launchpad.net/ubuntu/+source/snapd/+bug/1849753) .
...
Secondly, you can install chezmoi with any of the [many supported install methods](https://www.chezmoi.io/install/) instead of snap.
