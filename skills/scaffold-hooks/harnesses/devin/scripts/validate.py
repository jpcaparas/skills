...
Validate the devin harness component of scaffold-hooks harness component structure and event manifest.
...
def validate_manifest(skill_path: Path, errors: list[str], warnings: list[str]) -> None:
    ...
    if ".devin/hooks.v1.json" not in "\n".join(manifest.get("notes", [])):
        errors.append("assets/hook-events.json must document .devin/hooks.v1.json")
