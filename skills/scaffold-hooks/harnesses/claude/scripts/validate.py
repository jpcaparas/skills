...
Validate the claude harness component of scaffold-hooks harness component structure and its hook-event manifest.

Usage:
    python3 validate.py <harness-path>
...
def parse_frontmatter(content: str) -> tuple[dict | None, str]:
    """Extract a simple YAML frontmatter block from a markdown file."""
...
def extract_file_references(content: str) -> list[str]:
    """Extract referenced local files from markdown while skipping code fences."""
...
def validate_manifest(skill_path: Path, errors: list[str], warnings: list[str]) -> None:
    """Validate the baked-in hook-event manifest."""
    ...
    if len(events) != EXPECTED_EVENT_COUNT:
        errors.append(
            f"assets/hook-events.json should contain {EXPECTED_EVENT_COUNT} events; "
            f"found {len(events)}"
        )
    ...
    if len(set(event_names)) != len(event_names):
        errors.append("assets/hook-events.json contains duplicate event names")
    if len(set(script_names)) != len(script_names):
        errors.append("assets/hook-events.json contains duplicate script_name values")

    if not manifest.get("verified_from"):
        warnings.append("assets/hook-events.json has no verified_from sources")


def validate_skill(skill_path: Path) -> dict:
    """Run all structural checks for the skill."""
    errors: list[str] = []
    warnings: list[str] = []
