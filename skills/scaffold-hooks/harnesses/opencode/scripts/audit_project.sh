...
# audit_project.sh
#
# Inspect a target project and report OpenCode-hook-relevant facts as JSON.
#
...
if printf '%s' "$opencode_files_json" | jq -e '. | any(startswith(".opencode/"))' >/dev/null; then
    RECOMMENDED_SCOPE="project"
fi
if printf '%s' "$opencode_files_json" | jq -e '. | index("opencode.json") or index("opencode.jsonc")' >/dev/null; then
    RECOMMENDED_SCOPE="project"
fi

RECOMMENDED_DEPLOYMENT="local-files"
if printf '%s' "$opencode_files_json" | jq -e '. | index("opencode.json") or index("opencode.jsonc")' >/dev/null; then
