#!/usr/bin/env python3
"""
test_skill.py - Validate packaging and run eli12 probes.
...
def run_tests(skill_path: str) -> dict[str, object]:
    ...
    markdown_files = [root / "SKILL.md", root / "README.md", root / "AGENTS.md"]
    markdown_files.extend((root / "references").rglob("*.md"))
    markdown_files.extend((root / "templates").rglob("*.md"))
    markdown_files.extend((root / "agents").rglob("*.md"))
