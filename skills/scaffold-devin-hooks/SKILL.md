...
- Existing Claude hook config is present:
  Treat it as an inherited-config risk to inspect, not as the target. Do not write generated Devin hooks to Claude config paths.
...
| Merge generated hooks into `.devin/hooks.v1.json` | Let `scripts/scaffold_hooks.sh` call `scripts/merge_hooks_file.sh`, or run the merge script directly |
...
6. Prefer `.devin/hooks.v1.json` for generated project hooks because Devin documents it as the recommended standalone hooks file.
7. Avoid generated Claude config. Do not write managed Devin hooks to `.claude/settings.json`, `.claude/settings.local.json`, `~/.claude.json`, or any Claude-only path.
8. Prefer repo-owned shared scripts for behavior that may move to Codex, OpenCode, Git hooks, GitHub Actions, or local shell usage. Keep generated Devin hook files as thin adapters around those scripts.
9. Scaffold every current documented Devin lifecycle event as a commented bash stub under the managed hook root, even if the event stays disabled in `.devin/hooks.v1.json`.
...
12. Regenerate `.devin/hooks/README.md` so the project always has a readable event map.
...
- existing Claude config only as inherited-hook context; do not use it as the generated target
...
- Use `$DEVIN_PROJECT_DIR` in generated command paths because Devin documents it as the project root environment variable.
- Default to a managed root of `.devin/hooks/generated`.
...
- `scripts/render_hooks_readme.sh` rebuilds `.devin/hooks/README.md` from the manifest and current plan.

## Gotchas

1. `.devin/hooks.v1.json` is the whole hooks object; do not wrap generated hooks under a top-level `"hooks"` key there.
