...
# audit_project.sh
#
# Inspect a target project and report Devin-hook-relevant facts as JSON.
#
...
RESULT_JSON="$(
    jq -n \
        ...
        --arg recommended_hooks_target ".devin/hooks.v1.json" \
        --arg managed_root "$MANAGED_ROOT" \
        --argjson is_git_repo "$IS_GIT_REPO" \
        --argjson marker_files "$marker_files_json" \
        --argjson devin_files "$devin_files_json" \
        --argjson claude_inherited_files "$claude_inherited_files_json" \
        ...
        '
        {
            ...
            devin: {
                files: $devin_files,
                recommended_target: $recommended_hooks_target
            },
            inherited_claude_config: {
                files: $claude_inherited_files,
                note: "Inspect for inherited behavior only. Do not write generated Devin hooks here."
            },
            command_sources: {
