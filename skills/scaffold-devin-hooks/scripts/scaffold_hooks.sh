...
case "$MANAGED_ROOT_REL" in
    .devin/hooks/generated|.devin/hooks/generated/*)
        ;;
    *)
        echo "Managed root must stay under .devin/hooks/generated. Got: $MANAGED_ROOT_REL" >&2
        exit 1
        ;;
esac
...
mkdir -p "$PROJECT_ROOT/.devin/hooks"
...
# common.sh
#
# Shared helper functions for generated Devin CLI hook scripts.
#
...
require_jq() {
    ...
    if ! command -v jq >/dev/null 2>&1; then
        echo "jq is required by generated Devin hook helpers." >&2
        return 1
    fi
}

read_hook_input() {
    # Devin passes one JSON object to command hooks on stdin.
...
hook_json() {
    ...
    #
    # Arguments:
    #   $1 - jq filter, for example '.tool_name'
...
block_action() {
    # Devin documents exit code 2 as the intentional block/deny signal.
    local reason="$1"
...
hook_project_root() {
    # Prefer Devin's documented project root environment variable, then git,
    # then the process cwd. This keeps command execution repo-local.
...
run_project_command() {
    ...
    printf '[devin-hook] %s\n' "$label" >&2
    printf '[devin-hook] cwd: %s\n' "$cwd" >&2
    printf '[devin-hook] command: %s\n' "$command" >&2
...
run_project_script() {
    ...
    # when the same behavior may also be called by Codex, OpenCode, Husky, CI,
    ...
    if [ ! -f "$resolved_script" ]; then
        printf '[devin-hook] missing script: %s\n' "$resolved_script" >&2
        return 127
    fi

    printf '[devin-hook] %s\n' "$label" >&2
    printf '[devin-hook] cwd: %s\n' "$cwd" >&2
    printf '[devin-hook] script: %s\n' "$resolved_script" >&2
...
run_configured_scripts() {
    ...
    #   [{"label":"stop checks","path":"scripts/agent-stop-checks.sh","args":["devin"],"cwd":"."}]
    #
...
handle_configured_failure() {
    # Convert failed project commands into Devin's documented blocking contract
    # only when the plan says failure should block. Other errors are logged by
    # Devin and do not block according to the official exit-code table.
    local event_name="$1"
                ...
                (
                    {
                        hooks: [
                            (
                                {
                                    type: "command",
                                    command: ("bash \"$DEVIN_PROJECT_DIR/" + $managed_root + "/events/" + $event.script_name + "\"")
                                }
