#!/usr/bin/env python3
"""Lightweight tests for the mockable-code skill."""
...
export async function chargeCustomer(input: any) {
  ...
  const response = await fetch("https://payments.example.test/charge", {
    method: "POST",
    body: JSON.stringify({ input, requestId, startedAt, client })
  });
...
def main(argv: list[str]) -> int:
    ...
    help_check = subprocess.run(
        [sys.executable, str(root / "scripts" / "analyze_mockability.py"), "--help"],
        text=True,
        capture_output=True,
        check=False,
    )
    if help_check.returncode != 0 or "mockability review prompts" not in help_check.stdout:
        errors.append("analyze_mockability.py --help did not return expected help text")
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        scan = subprocess.run(
            [
                sys.executable,
                str(root / "scripts" / "analyze_mockability.py"),
                str(fixture_root),
                "--json",
            ],
            text=True,
        ...
        if scan.returncode != 0:
            errors.append("analyze_mockability.py failed on fixture")
        else:
    ...
    template = root / "templates" / "mockability-review.md"
    if not template.is_file():
        errors.append("mockability review template is missing")
