#!/usr/bin/env python3
"""Validate the devils-advocate skill package."""
...
REQUIRED_CONTRACTS = {
    "Use this skill only when": "explicit invocation boundary",
    "temporary opposing position": "temporary opposition role",
    "Steelman the idea before challenging it": "steelman-first behavior",
    "Do not invent facts": "evidence integrity",
    "Concede a point": "honest concession behavior",
    "Do not insult": "person-versus-idea boundary",
    "asks to stop": "stopping behavior",
}
ASSERTION_TYPES = {"functional", "structural", "negative", "verification", "disclosure"}
REQUIRED_EVAL_TAGS = {"smoke", "edge", "negative", "debate", "evidence", "safety", "invocation"}
...
def validate_triggers(root: Path, errors: list[str]) -> int:
    ...
    if not any("devils-advocate" in query for query in positive_queries):
...
def validate_skill(root: Path) -> ValidationResult:
    ...
    if skill_path.is_file():
        ...
        if frontmatter.get("description") != "Debates when explicitly invoked.":
            errors.append("frontmatter description must preserve the active invocation contract")
        if skill_lines > 500:
    ...
    if manifest_path.is_file():
        ...
        if not re.search(r"(?m)^policy:\s*\n\s+allow_implicit_invocation:\s*false\s*$", manifest):
            errors.append("agents/openai.yaml must disable implicit invocation")

    prompt_path = root / "skill-card.prompt.md"
    if prompt_path.is_file() and "`devils-advocate` README skill badge" not in prompt_path.read_text(encoding="utf-8"):
        errors.append("skill-card.prompt.md must identify devils-advocate")
