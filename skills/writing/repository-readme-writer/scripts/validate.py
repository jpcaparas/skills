#!/usr/bin/env python3
"""Validate the repository-readme-writer skill structure and references."""
...
def validate_skill(skill_path: str | Path) -> dict:
    ...
    for relative in ["SKILL.md", "references/foundations.md", "references/repository-audit.md"]:
        path = root / relative
        if path.is_file():
            text = path.read_text(encoding="utf-8")
            if "TODO:" in text:
