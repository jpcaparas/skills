#!/usr/bin/env python3
"""
validate.py - Validate the client-report-from-commits skill structure and conventions.
...
def validate_skill(skill_path: str) -> dict[str, object]:
    ...
    if refs_root.is_dir():
        for ref_path in refs_root.rglob("*.md"):
            ...
            if lines > 1000:
                errors.append(
                    f"Reference file exceeds 1000 lines: {ref_path.relative_to(root)}"
                )
            elif lines > 300 and not has_toc_heading(ref_content):
                warnings.append(
                    f"Reference file >300 lines without TOC: {ref_path.relative_to(root)}"
                )
