#!/usr/bin/env python3
"""Validate the repo-intent-documenter skill structure and references."""
...
REQUIRED_FILES = [
    ...
    "scripts/repo_intent_inventory.py",
    "scripts/test_repo_intent_inventory.py",
    "scripts/validate.py",
...
def validate_skill(skill_path: str | Path) -> dict:
    ...
    for path in sorted((root / "references").glob("*.md")):
        ...
        refs = extract_file_references(ref_content)
    ...
    for relative in ["SKILL.md", "references/methodology.md", "references/document-contract.md", "references/gotchas.md"]:
        path = root / relative
        if path.is_file():
            text = path.read_text(encoding="utf-8")
