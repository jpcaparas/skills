...
- Subject-adapted depth guidance when refinement is allowed; exact-prompt requests remain byte-for-byte with no compulsory additions
...
- Explicit outer fan-out for multiple leads, workspaces, and same-prompt replicas, with optional user-requested design territories and strict sibling blindness
...
- Explicit pre-dispatch choice: the existing quality gauntlet, or generation only with no artifact/workspace verification and a clearly UNVERIFIED handoff; immutable receipt mode survives recovery
- An opted-in lead-owned quality gauntlet with mobile-friendliness inspection, consolidated bar-and-artifact review, terminal `READY`, batched material blockers, targeted rechecks, and evidence reuse
...
- Live-first public `GET` interfaces with bundled build-time snapshots and explicit freshness states; fallback testing only when gauntlet is selected
- Mouse-and-keyboard-friendly games and simulations; opted-in gauntlet runs add transient probe contracts, production-state adapters, transformed-viewpoint checks, and coordinator-owned browser evidence
