...
def exercise_browser_clock(skill: Path, errors: List[str]) -> None:
    """Run the real-browser clock, input, and error-lifecycle contract tests."""
    result = run([sys.executable, str(skill / "scripts" / "test_browser_session.py")])
    assert_ok(
        result.returncode == 0,
        f"controlled-clock browser regressions failed: {result.stdout}{result.stderr}",
        errors,
    )
