...
## Verification Consent and Generation-Only Branch

Before reserving or dispatching a new build, ask whether the user wants the existing battery of tests (the gauntlet), unless the active request explicitly chooses. Silence is not consent. Pass the choice explicitly as `prepare_run.py --verification gauntlet|none`. This produces run schema `3.5`, receipt `2.5`, and worker report `2.1` with matching `verificationMode`. The immutable coordinator receipt anchors the selection; no worker-only change can opt out. Legacy helper calls without the flag keep 3.4/2.4 gauntlet behavior; historical missing mode is never none.

With `none`, generate and export only. No artifact/workspace verification: no browser/render/screenshots, smoke tests, lints/typechecks/tests, critics, directional probe or adapter, fallback-path checks, benchmarks, static scans, or validation/repair loops. Essential build/export commands are allowed, but their success is not behavioral evidence. All checking, measurement, and gauntlet guidance below is conditional on `gauntlet`. In none mode retain provenance identity reads, scoped authority, single writer, prompt preservation, and safe cleanup only; do not use them to inspect output. Resume existing source only as needed to continue generation, not to judge the artifact.

Dispatch the exact receipt mode and these prohibitions to every lead and descendant. Omit critics and directional guidance in none mode; do not create or require `.tmp/TECHNICAL_PROMPT.md`, including on recovery. Preserve mode unchanged on reconnect or replacement. Completed generation-only output is `UNVERIFIED` with `qualityGauntlet: null`, `verification: []`, and `artifact.staticDeploymentVerified: false`, never `OK`. In this reference, completed includes both `OK` and `UNVERIFIED`; neither retains scratch after finalization. Active or failed outcomes retain `.tmp/`. Package regression tests and repository validation remain required.

For none-mode finalization, stop all writers, promote durable output, keep both statuses `RUNNING`, then invoke the safe `cleanup_run_tmp.py --run <exact-run-path> --confirm-finalized` helper. It checks receipt/prompt identity, mode, metadata, and deletion boundaries without scanning generated content. Only after success set both statuses `UNVERIFIED`. Do not run `validate_catalog.py` as a completion gate. Build the navigation index if desired without opening the result. An explicitly requested catalogue metadata check skips none-mode artifact contents and cannot upgrade their status.
...
- the receipt-anchored verification mode and its operational envelope
- `agents/oneshot-critic.md` only for gauntlet, included as operational role material for descendants rather than relying on ambient package discovery
...
- only this run’s private design territory when the user requested variations, or an explicit not-applicable marker otherwise
...
For an unmatched custom request, refine faithfully only when the user permits refinement. Preserve the user’s constraints and exact wording requirements, clarify the core experience, and add only experience-level guidance that follows from the request. Do not borrow from the catalogue when there is no genuine match, and do not compress the prompt to an arbitrary paragraph or token target. Store and dispatch that text exactly. When the user requires their entire source brief to remain verbatim, preserve it byte-for-byte as the entire actual prompt, with no appended creative requirements. Exact-prompt requests override the suggested experience additions below; safety and provenance stay in the operational envelope. Probe schemas and machine interfaces never belong in the sealed prompt.

When refinement is allowed, use the catalogue’s `completionMandate` as subject-specific guidance within the user's requested scope, not a compulsory addition to an exact prompt. Reject shortcuts and cookie-cutter approximations without mentioning skill policy, token budgets, subagents, or internal workflow. For replicas, clones, and emulators, preserve source fidelity; do not invent differences. Do not paste the literal root value as boilerplate. The exact finished prompt is what the lead receives and what `artifact/PROMPT.md` preserves.

If the experience depends on unauthenticated public HTTP `GET` data and refinement is allowed, make resilience part of that finished actual prompt. Ask for build-time local snapshots for the public responses needed by the meaningful default or primary experience even when live CORS works today, schema-valid live data at runtime, and snapshots after a timeout, network or DNS failure, restrictive CORS policy, non-success response, malformed payload, or schema mismatch. A first-visit HTTP or browser cache is not a substitute for data already bundled in the artifact. Ask for honest source and capture-time disclosure where freshness matters. Keep live-success plus forced-fallback verification out of the sealed brief; it is gauntlet-only operational work. Large and volatile feeds still qualify: a task-relevant bounded slice is acceptable when the complete feed is disproportionate, provided the interface does not overstate its coverage or freshness. Do not bundle secrets, authenticated or private responses, personal or sensitive data, or content without redistribution rights; use an explicit unavailable or empty state when a local copy would be inappropriate. Do not add this requirement to a brief with no public `GET` dependency.
...
- Only user-requested variations receive private design territories. Every outer lead remains blind to sibling territories, workspaces, artifacts, evidence, critics, and outcomes, including identical replicas.
...
The plan is valid when the number of distinct top-level lead owners equals the number of requested experiment instances, all slugged timestamp run paths are disjoint, every same-brief replica has the same prompt digest and byte count, and territories are supplied only for requested variations.

## Blind Design Diversity

Only for user-requested design variations, create a coordinator-only private design-diversity ledger. Keep the same sealed prompt for every repeated brief and assign positive design territories within the requested discretionary axes. Give each lead only its own territory. Do not reveal the sibling count, names, territories, workspaces, artifacts, captures, reports, critics, outcomes, or comparative feedback, and do not let descendants inspect sibling paths even when a readable parent directory makes discovery technically possible. Do not share sibling work to coordinate differentiation. Identical replicas have no forced diversity and may receive the same user-supplied reference independently.

The territory governs only discretionary design choices and stays outside the actual prompt and `artifact/PROMPT.md`. A fixed source, user-mandated identity, or faithful-replica constraint may require common traits; do not distort them for artificial variety or block identical replicas for lacking design freedom. Persist an assigned territory only in that run’s `worker-report.json.observations.designTerritory` and pass only it down that lead’s descendant and critic tree. A continuation or sequential recovery receives the same private territory unchanged.
...
This entire section applies only to gauntlet mode. Generation-only runs skip it, including bar scoring, integration inspection, critics, and fallback evidence.
