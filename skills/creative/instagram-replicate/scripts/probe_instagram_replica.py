#!/usr/bin/env python3
"""Run live end-to-end checks against instagram-replicate."""
...
def run_probe(source: str, save_root: Path, cleanup: bool, gif_max_bytes: int) -> dict[str, Any]:
    script_path = Path(__file__).resolve().parent / "render_instagram_replica.py"
    command = [
        sys.executable,
        str(script_path),
        source,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("sources", nargs="+", help="Public Instagram post/reel URLs to test")
    parser.add_argument(
        "--save-root",
        required=True,
