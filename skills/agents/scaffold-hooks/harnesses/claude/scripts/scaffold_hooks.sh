...
config_collection_json() {
    ...
    # Preserve an explicitly configured null or wrong type so the runner can
    # reject it. Treat only an absent field as an empty collection.
...
config_scripts_json() {
    config_collection_json "scripts"
}

config_commands_json() {
    config_collection_json "commands"
}
...
config_code_change_extensions_json() {
    config_collection_json "code_change_extensions"
}
...
if ! MANIFEST_ROWS="$(
    jq -er '.events[] | [.name, .script_name, .description, .async_guidance] | @tsv' \
        "$MANIFEST_SOURCE"
)"; then
    echo "Failed to read complete Claude hook manifest rows." >&2
    exit 1
fi
