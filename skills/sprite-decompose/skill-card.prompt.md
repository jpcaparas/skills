Nano Banana 2 image generation prompt for the `sprite-decompose` README skill badge.
...
Subject: a luminous illustrated mosaic passing through a precision cutting gate and separating into several clean transparent sprite gems, with exact crop rails and a small manifest crystal.
...
Blank-surface rule: any book, ticket, screen, terminal, dashboard, chart, browser, sign, speech bubble, social post, calendar, spreadsheet, map, or interface surface must be visually blank or represented only by solid unlabeled rectangles, bars, dots, connectors, and simple icons. Do not draw interior strokes that resemble writing.
...
No visible text, no letters, no numbers, no typography, no captions, no labels, no logos, no watermarks, no pseudo-text, no glyph-like marks, no UI copy, no code characters, no punctuation, no checkmarks, no question marks.
