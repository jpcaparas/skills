...
Freeze a public X/Twitter post into a rerenderable local build that always emits both `tweet-replica.mp4` and `tweet-replica.gif`, with the GIF kept under 24 MB.
...
- download the avatar and primary media locally
...
| Task | Command | Why |
| --- | --- | --- |
| Render from a status URL into the default designated save root | `python3 scripts/render_tweet_replica.py 'https://x.com/.../status/...' --save-root ./pieces` | Creates a rerenderable build folder under `./pieces/` |
...
| Keep everything in one named build folder | `python3 scripts/render_tweet_replica.py '<url>' --workdir ./tweet-build` | Leaves snapshot, local assets, HTML, WebM, MP4, and GIF together |
...
Treat the snapshot as the truth, not the live tweet page. Fetch once, freeze the data, then iterate locally from `snapshot.json`.
...
5. Playwright's viewport recording is visual-only. The final MP4 muxes original media audio back in when a local video file has an audio stream. The GIF never carries audio.
