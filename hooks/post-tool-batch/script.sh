...
# script.sh
#
# Canonical event script for PostToolBatch.
#
# What this event is for:
#   Runs once after every tool call in a parallel batch resolves and before the next model request.
#
# Recommended starting style:
#   Use sync when batch-level context must reach Claude before the next model call; use async only for passive side effects.
#
...
: "${AGENT_HOOK_EVENT:=PostToolBatch}"
