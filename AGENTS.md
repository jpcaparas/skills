...
Native validation also requires Bun, Node.js with `npx`, Git, `jq`, and ripgrep (`rg`). Put compatible executables on `PATH`, or select non-standard installations with `SKILLS_VALIDATION_PYTHON`, `SKILLS_VALIDATION_BUN`, `SKILLS_VALIDATION_NODE`, and `SKILLS_VALIDATION_NPX`. These userland runtimes intentionally accept compatible versions; the hosted workflow remains deterministic and currently pins Python 3.11, Node.js 24, and Bun 1.3.11. Every GitHub Action in `.github/workflows/` is pinned to an immutable commit SHA with a `# vX.Y.Z` comment recording the tag it belongs to, and `scripts/check-validation-entrypoint-parity.py` enforces the exact pinned refs; Dependabot (`.github/dependabot.yml`) proposes update PRs, so bump the SHA, its comment, and the parity-check constant together.
